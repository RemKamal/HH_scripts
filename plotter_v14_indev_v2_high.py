...
#        for key,value in ordDict.items():
 #           print(key, ":", value)
  #          print 'type(key) is ', type(value[2]) 
        ...
        outF = ROOT.TFile.Open('hists_' + bin + extension + '.root','recreate')
        binStr =         'bin                                            '
        ...
        #eff_b_lf_shapeLine = 'vhbb_eff_b_lf                            shape '
        #eff_b_hf_shapeLine = 'vhbb_eff_b_hf                            shape '
        JEC_shapeLine  =  'vhbb_scale_j                             shape '
        #JER_shapeLine  =  'vhbb_res_j                               shape '
        ...
        for h in ordDict:
            if 'data' not in h:
                ...
                # eff_b_lf_shapeLine += '1'.ljust(padding)
                # eff_b_hf_shapeLine += '1'.ljust(padding)
   ...
   #         elif 'SR' in regionType:
             ...
             #   print 'smth is wrong writing parapLine to datacard, please check.'
              #  sys.exit(1)
                ...
                fOut.write('\n')
                fOut.write('\n')
                #fOut.write('#  shape SYSTEMATICS\n')
                #fOut.write('\n')
#                fOut.write(eff_m_shapeLine)
                #fOut.write('\n')
 #               fOut.write(eff_b_lf_shapeLine)
                #fOut.write('\n')
   #             fOut.write(eff_b_hf_shapeLine)
                #fOut.write('\n')
...
#                if 'h' in plotName:
 #                   plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio = dataOrNot, plotData = dataOrNot)
  #              else:
        ...
        if 'makeDataCards' in opt.kind:
            plots[p].appendTo(outDir+'/{0}.input.root'.format(channel2run+ '_' + regionType + '_' + systUnc), makeDataCards=True, channel2run = channel2run, regionType=regionType)
        else:
