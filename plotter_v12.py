...
#//////////////////////////
    def makeDataCards(self, outDir,lumi,noScale=False,saveTeX=False, regionType=None, zzFactor=1, wwFactor=1):
        dataDict = {}
        printStrings = True
        ...
        processName = 'hh'
        zz_signalMultFactor = zzFactor * 1.
        ww_signalMultFactor = wwFactor * 1.
        ...
        for h in ndic:
            ...
            if 'zz' in ndic[h].GetName():
                ...
                ndic[h].Scale(1./zz_signalMultFactor)
                if fix:
                    ndic[h].Scale(zzfix)  
            elif 'ww' in ndic[h].GetName():
                wwRate_unscaled = ndic[h].Integral()
                ndic[h].Scale(1./ww_signalMultFactor)
                if fix:
                    ndic[h].Scale(wwfix)
            else:
                pass
 ...
 #                   plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio = dataOrNot, plotData = dataOrNot)
  #              else:
                ...
                plots[p].overlayHists(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX)
                    ...
                    plots[p].makeDataCards(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, regionType=regionType, zzFactor = opt.zzfactor, wwFactor = opt.wwfactor)
            else:
