...
def runSimplestAn(inFileURLorList, outFileURL, xsec=None, denominator=None, isSignal = False, splitSignal = None, whichChannel = 0):#muons):
    ...
    binLabels = ['no cut', '>=2b-jets', '90]Hbb[150',  '2leps', '76<Z<106', 'met>30', 'hhMt>250']
    lastEffBin = 8
    ...
    histos={
        ...
        'btagCSV0'       :ROOT.TH1F('btagCSV0'	,';Leading b-Jet CSV weight; Events'	    , 40, -1., 1.),
        'btagCSV1'       :ROOT.TH1F('btagCSV1'	,';Subleading b-Jet CSV weight; Events'	    , 40, -1., 1.),
       ...
       # 'hCSVmass_raw'      :ROOT.TH1F('hCSVmass_raw', ';HCSV mass [GeV]; Events', 70, 25, 165),
      ...
      #  'hhMass'      :ROOT.TH1F('hhMass', ';"HiggsHiggson" mass [GeV]; Events', 40, 500, 900),
        'hhMt_long'      :ROOT.TH1F('hhMt_long', ';"HiggsHiggson" transverse mass [GeV]; Events', 90, 200, 1100),
       # 'zMass_raw'      :ROOT.TH1F('zMass_raw', ';Z mass [GeV]; Events', 70, 50,120),
...
##########vs     
        ...
        # start HH selection 
        #*******************
        bazinga('asked for at least 2b-jets')
        ...
        if tree.HCSV_mass >= 90 and tree.HCSV_mass <= 150: continue 
...
# tree->Scan("tree.Jet_btagCSV[tree.hJCidx[0]]")
        ...
        histos['hZZpt'].Fill(hToZZ.Pt(),evWgt)                 
        histos['hZZmt'].Fill(hToZZ.Mt(),evWgt)                 
        histos['hZZmt_long'].Fill(hToZZ.Mt(),evWgt)                 
        ...
        histos['hCSVmass'].Fill(tree.HCSV_mass,evWgt)    
        histos['hCSVmass_long'].Fill(tree.HCSV_mass,evWgt)
        histos['zMass_long'].Fill(tree.V_mass, evWgt)
        histos['hCSVpt'].Fill(tree.HCSV_pt,evWgt)    
...
#        histos['cutFlow'].Fill(4,1)    
        ...
        #if tree.Jet_pt[0] > tree.Jet_pt[1]:
...
#        hh_mass = computeHHMass(Z_p4, missing_p4, Htobb_p4)
     #   histos['hhMass'].Fill(1,evWgt)    
