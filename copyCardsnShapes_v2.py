...
def parallCall(cmd):
    ...
    outDir = leptType + str(massRegion) + '_' + str(bdtCut) + date + str(mass)
...
pool.map(parallCall, nomin_ee_low_params )
pool.map(parallCall, nomin_mm_low_params )
pool.map(parallCall, nomin_ee_high_params )
pool.map(parallCall, nomin_mm_high_params )

pool.map(parallCall, ee_low_params )
pool.map(parallCall, mm_low_params )
pool.map(parallCall, ee_high_params )
pool.map(parallCall, mm_high_params )
...
cpCommand = 'cp -r ' + '{mm,ee}'  + '_*' + date[:-1] + ' ' + dest  
print cpCommand
print 
print 
if trueRun:
