    ...
    def makeDataCards(self, outDir,lumi,noScale=False,saveTeX=False):
        ...
        for h in ndic:
            ...
            if 'sig' in ndic[h].GetName():
                ndic[h].Scale(1./signalMultFactor)
 ...
 #           print(key, ":", value)
  #          print 'type(key) is ', type(value[2]) 
        ...
        if os.path.exists(outDir):
            os.chdir(outDir)
        outF = ROOT.TFile.Open(processName + '_Mt.root','recreate')
        binStr =         'bin          '
        ...
        outF.Close()
        #encoding='utf-8'
        with io.open ('dataCard_' + processName +'.txt', mode = 'wb') as fOut:
            ...
            fOut.write('# or\n')
            fOut.write('# shapes * * hh_Mt.root $CHANNEL/$PROCESS $CHANNEL/$PROCESS_$SYSTEMATIC\n')
            fOut.write('shapes * * hh_Mt.root $PROCESS $PROCESS_$SYSTEMATIC\n')
            fOut.write('#-----------------------------------------------------------------------------------\n')
            ...
            fOut.write('#~/workspace/private/feb22HH/CMSSW_8_0_25/src/VHbbAnalysis/Heppy/test/analysis_apr01_tot_hzz_CRDY_v5\n')
            fOut.write('#-----------------------------------------------------------------------------------\n')
            fOut.write(binStr)
            fOut.write('\n')
...
#            fOut.write('# lumi_13TeV  lnN             1.06       1.06      -       -       -       -       -
# bTag      lnN               1.10       1.10      -       -       -       -       -
