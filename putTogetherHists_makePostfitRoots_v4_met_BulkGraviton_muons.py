...
doNotProduceBDTAfter = False

doNotRebinBDT = True
rebinByTemplate = True
...
def modifyDatacard(var, datacards):
    ...
    if doNotProduceBDTAfter and 'after' in var: return
...
def copyAndCombineCards(varbl):
    ...
    if doNotProduceBDTAfter and 'after' in varbl: return
...
def separateFilesPerVariable(files, rebin):
    ...
    for var in variables:
        ...
        if doNotProduceBDTAfter and 'after' in var: continue
...
def putTogetherHists(files, rebin):
    ...
    for f in files:
        ...
        for idx, shape in enumerate(shapes):
            ...
            if (rebin > 1 or rebinByTemplate) and 'SR' in innerDir:
...
#                elif channel == "mm" and int(mass) == 650:                                                                                                                     
 #                   rebin_factor = 20                                                                                                                                          
                ...
                if 'hhMt' in varName:
                    if int(mass) < 300:
                        xbins_list += [310, 350, 390, 430, 470, 490, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
                    elif int(mass) == 300:
                        xbins_list += [270, 310, 350, 390, 430, 470, 510, 550, 590, 630, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
                    elif int(mass) == 350:
                        xbins_list += [270, 310, 350, 390, 430, 470, 510, 550, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]

                    elif int(mass) in [400, 450, 451] and channel == 'mm':
                        xbins_list += [550, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
                    else:
                        xbins_list += [shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
                    xbins = array("d", xbins_list)
...
#                 print 'shape.GetName()', shape.GetName()
#                 print 'int(mass)', int(mass)
#                 if 'hhMt' in varName and int(mass) > 451 and int(mass) not in [600]:
#                   xbins_list = [shape.GetBinLowEdge(1)]
#                   xbins_list += [600, 800, 1000, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
#                   xbins = array("d", xbins_list)
                   
#                 elif 'hhMt' in varName and int(mass) == 600:
#                     xbins_list = [shape.GetBinLowEdge(1)]
#                     xbins_list += [600, 800, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
#                     xbins = array("d", xbins_list)

#                 elif 'hhMt' in varName and 300 <= int(mass) <= 450:
#                     xbins_list = [shape.GetBinLowEdge(1)]
#                     xbins_list += [425, 550, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
#                     xbins = array("d", xbins_list)

#                 elif 'hhMt' in varName and int(mass) <= 270:
#                     xbins_list = [shape.GetBinLowEdge(1)]
#                     xbins_list += [425, 550, shape.GetBinCenter(nbins) + shape.GetXaxis().GetBinWidth(nbins)/2]
#                     xbins = array("d", xbins_list)
#                 else:
#                     pass
            ...
            elif (rebin > 1 or rebinByTemplate) and 'CR' in innerDir:
                if 'hh' in varName:
                    print 'not a bdt, rebin'
                    shape.Rebin(5) #rebin_factor)
            else:
                pass
