...
class NewVLeptons:
   ...
   def __init__(self, pt, eta, phi, mass, relIso03, relIso04):
       self.pt = pt
       self.eta = eta
       self.phi = phi
       self.mass = mass
...
class Event(TreeModel):
    ...
    selLeptons_relIso03 = FloatArrayCol(10)
    selLeptons_relIso04 = FloatArrayCol(10)
...
#print 'tree is ', str(tree)
    ...
    if doVtypeCorrection:
        vtc = VtypeCorrector(tree, channel='zll')
        #vtc = VtypeCorrector(channel='zll')
        print 'doing doVtypeCorrection'
    ...
    for i in xrange(0,int(totalEntries)):
        ...
        if doVtypeCorrection:
            ...
            #new_vtype = vtc.processEvent(tree)
            if new_vtype != tree.Vtype:
                print 'misVtype found:'
                print 'old vtype', tree.Vtype
        ...
        new_vtype = None #tree.Vtype


        lepsP4 = []
        print 'create objects of new classes'
        if recomputeVtype:
            newVLeptons = NewVLeptons(-1,-1,-1,-1,-1,-1)
        ...
        if (whichChannel == 0 or whichChannel ==1 ) and recomputeVtype:
            ...
            if len(zMuons) >=  2 or len(zElectrons) >=  2:
                for i in range(len(tree.selLeptons_mass)):
                    print 'tree.selLeptons_mass[i]', tree.selLeptons_mass[i]
            ...
            #print 'newLepton1', newLepton1
            if Vtype_new_ == None:
                print 'check Vtype_new_, exiting', 
                sys.exit(1)

                    # skip event, if vtype_new doesn't correspond to whichChannel
            if whichChannel in [0, 1]:#'Zll':
                if Vtype_new_ != 0 and Vtype_new_ != 1:
                    n_vtype_events_skipped += 1
                    continue
                #elif whichChannel == 'Wlv':
                 #       if Vtype_new_ != 2 and Vtype_new_ != 3:
                  #          n_vtype_events_skipped += 1
                   #         continue
                    #elif whichChannel == 'Zvv':
                     #   if Vtype_new_ != 4:
                              #n_vtype_events_skipped += 1
                              #continue
        ...
        minitree.newVtype = new_vtype

        #print 'countWeighted is ', countWeighted
        
        if new_vtype ==1: #tree.Vtype == 1: #Int_t OR of ['HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v*', 'HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL_v*']*
            #if not tree.HLT_ttH_DL_elel: continue

            if not tree.HLT_BIT_HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v: continue  
            #as in vhbb AN

        elif new_vtype ==0: #tree.Vtype == 0:
...
#newLepton0 newLepton1  newVBoson                                                                                                               
        ...
        print 'while nvLeps', nvLeps
        #if len(lepsP4) <= nvLeps + 1 :
       ...
       # print 'after tmpt, bJetsP4 is ', bJetsP4
        ...
        if recomputeVtype:
            ...
            bazinga('newVBoson.mass is {0}'.format(newVBoson.mass) )
            if physRegion == "SR" and not passLoose:
            #print 'doing ', physRegion
                if newVBoson.mass <76 or newVBoson.mass > 106 : continue
            elif physRegion == "SR" and passLoose:
                bazinga('in passLoose with newVBoson.mass is {0}'.format(newVBoson.mass) )
            #if newVBoson.mass < 76: continue# or newVBoson.mass > 126 : continue   #CHANGED
                if newVBoson.mass < 76:# or newVBoson.mass > 126 : 
                    continue   #CHANGED
            elif physRegion == "CRTT":
            #print 'doing ', physRegion
                if newVBoson.mass >= 76 and newVBoson.mass <= 106 : continue
            elif physRegion == "CRDY":
            #print 'doing ', physRegion
                if newVBoson.mass <76 or newVBoson.mass > 106 : continue
            elif physRegion == "CRDY_1b":
...
# tree->Scan("tree.Jet_btagCMVAV2[tree.hJCidx[0]]")
        ...
        minitree.selLeptons_relIso03 = tree.selLeptons_relIso03
        minitree.selLeptons_relIso04 = tree.selLeptons_relIso04
        ...
        if recomputeVtype:
            ...
            minitree.lep0Iso03 = tree.vLeptons_pfRelIso03[0]
            minitree.lep1Iso03 = tree.vLeptons_pfRelIso03[1]
            minitree.lep0Iso04 = tree.vLeptons_pfRelIso04[0]
            minitree.lep1Iso04 = tree.vLeptons_pfRelIso04[1]
        ...
        for j, name in SFjsons.iteritems():
            ...
            if new_vtype ==0: #tree.Vtype == 0: #whichChannel ==0
            ...
            elif new_vtype ==1 :#tree.Vtype == 1: #whichChannel ==1
...
#============================
    ...
    bazinga('done with entries, about to be storing minitrees')
    if fIn.IsOpen():
        fIn.Close()
