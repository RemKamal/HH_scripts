...
createCombineArea = 'June3'
prefDir = 'analysis_May25_%s_BGrav_total_SR_minitrees_inpb_wBR' 
    ...
    for mass in masses:
        ...
        #for typ in 'CRDYlow_', 'CRDY_', 'CRDYhigh_', 'good_SR_bdt_sideBands':
            ...
            if mass == 451:
                tmp_mass = 451
                mass = 450
            txts =  'find *nom*/%s_{SR_,CRDY_,CRTT_}*%s/*/make*/%s/*txt  -exec cp {}    ../%s  \;' % (region, str(bdtCut), str(mass), path)
            roots = 'find     */%s_{SR_,CRDY_,CRTT_}*%s/*/make*/%s/*root -exec cp {}    ../%s  \;' % (region, str(bdtCut), str(mass), path)
