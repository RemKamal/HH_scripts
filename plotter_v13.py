...
class Plot(object):
    ...
    def add(self, h, title, color, isData, isSignal):
        ...
        if title.startswith('t'):
            ...
            #sampleType = 'signal_both'
        elif title.startswith('zzBGrav'):
            sampleType = 'signal_zz'
        elif title.startswith('vvBGrav'):
...
#////////////////////////
    ...
    def doStack(self, outDir,lumi,noScale=False,saveTeX=False, ratio=False, plotData = True, massPoint=0, bdtCut = 0.24, regionType = None):
...
#        leg = ROOT.TLegend(0.25, 0.86, 0.88, 0.94)  
        ...
        if 'bdt' in self.name and regionType == 'SR':
            zzFactor =             [100000]*3 + [65000]*3  + [35000]*4 #15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [300000]*3 + [300000]*3 + [100000]*4 #[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]
        elif 'hh' in self.name and regionType == 'SR':
            zzFactor =             [5000]*3 + [3000]*3  + [50]*4 #15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [25000]*3 + [15000]*3 + [300]*4 #[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]


        elif 'bdt' in self.name and regionType == 'CRDY':
            zzFactor =             [250000]*3 + [160000]*3  + [280000]*4 #15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [1200000]*3 + [600000]*3 + [2000000]*2 + [3000000]*2 #[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]
        elif 'hh' in self.name and regionType == 'CRDY':
            zzFactor =             [10000]*3 + [9000]*3  + [500]*4 #15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [50000]*3 + [25000]*3 + [1200]*2 + [2400]*2 #[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]


        elif 'bdt' in self.name and regionType == 'CRTT':
            zzFactor =             [40000]*3 + [32000]*3  + [50000]*2 + [35000]*2#15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [3000]*3 + [1500]*3 + [3000]*2 + [2200]*2#[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]
        elif 'hh' in self.name and regionType == 'CRTT':
            zzFactor =             [25000]*3 + [12000]*3  + [150]*2 + [150]*2 #15000., 20000., 50000., 100000., 150000., 500000., 1000000., 4000000., 5000000.]
            wwFactor = totFactor = [1200]*3 + [1000]*3 + [12]*2 + [12]*2#[500., 750., 1000., 2500., 5000., 7500., 25000., 50000., 200000., 250000.]
        ...
        vvFactor = wwFactor
        zz_signalMultFactor = zzFactor[idx] * 1.
        ww_signalMultFactor = wwFactor[idx] * 1.
        vv_signalMultFactor = vvFactor[idx] * 1.
        ...
        for hS in self.mcSignal:
            print 'self.mcSignal[hS].GetTitle() is ', self.mcSignal[hS].GetTitle()
            #if not noScale : 
            ...
            if 'ww' in self.mcSignal[hS].GetTitle():
                self.mcSignal[hS].Scale(ww_signalMultFactor)
                self.mcSignal[hS].Scale(branchingRatios[ww])
                fact = ww_signalMultFactor
                print 'for ww mass is {0} and factor is {1}'.format(massPoint, ww_signalMultFactor)
            elif 'zz' in self.mcSignal[hS].GetTitle():
                self.mcSignal[hS].Scale(zz_signalMultFactor)
                self.mcSignal[hS].Scale(branchingRatios[zz])
                fact = zz_signalMultFactor
                print 'for zz mass is {0} and factor is {1}'.format(massPoint, zz_signalMultFactor)
            elif 'vv' in self.mcSignal[hS].GetTitle():
                self.mcSignal[hS].Scale(vv_signalMultFactor)
                self.mcSignal[hS].Scale(branchingRatios[vv])
                fact = vv_signalMultFactor
                print 'for vv mass is {0} and factor is {1}'.format(massPoint, vv_signalMultFactor)
            else:
                fact = None
                print 'smth is wrong, exiting'
                sys.exit(1)
            print 'self.mcSignal[hS].Integral() is ', self.mcSignal[hS].Integral()
            print 'self.mcSignal[hS].GetMaximum() is ', self.mcSignal[hS].GetMaximum()

            #print '-'*50
            #print 'fact is {0} for {1}'.format(fact, self.name)
            #print '-'*50
            legStr = '{0}x{1}'.format(self.mcSignal[hS].GetTitle(), int(fact))
...
#//////////////////////////
    def makeDataCards(self, outDir,lumi,noScale=False,saveTeX=False, regionType=None, massPoint=None, applyBR = True):
        print 'applyBR is ', applyBR
        
        delZZnWW_keepVVonly = True
        delVV = False
        ...
        for k, v in ndic.items():
            print k
            print v
            if delZZnWW_keepVVonly:
                if 'zz' in k or 'ww' in k:
        ...
        for h in ndic:
            ...
            if 'ZH' in ndic[h].GetName():#[nameEnding_beg+1:]:
                ...
                dicRates[ndic[h].GetName()] = [-1, round(tmpRate, 2),ndic[h]]
            elif 'signal_vv' in ndic[h].GetName():#[nameEnding_beg+1:]:                              
                dicRates[ndic[h].GetName()] = [-2, round(tmpRate, 2),ndic[h]]
...
#        for key,value in ordDict.items():
 #           print(key, ":", value)
  #          print 'type(key) is ', type(value[2]) 
        ...
        if os.path.exists(outDir):
            os.chdir(outDir)
        outF = ROOT.TFile.Open(bin + '_Mt.root','recreate')
        binStr =         'bin                                            '
        ...
        vhbb_res_jLine = 'vhbb_res_j                                 lnN '
        vhbb_scale_jLine = 'vhbb_scale_j                               lnN '
        vhbb_trigger_METLine = 'vhbb_trigger_MET                           lnN '
        vhbb_eff_mLine = 'vhbb_eff_m                                 lnN '
        vhbb_eff_bLine = 'vhbb_eff_b                                 lnN '
        QCDscale_VHLine = 'QCDscale_VH                                lnN '
        QCDscale_ttbarLine  = 'QCDscale_ttbar                             lnN '
        QCDscale_VVLine = 'QCDscale_VV                                lnN '
        QCDscale_DYLine = 'QCDscale_DY                                lnN '


        qqbarList = ['DY', 'ST', 'VV', 'ZH']
        #ggList = ['signal_zz', 'signal_ww', 'TT']
        ...
        #encoding='utf-8'
        with io.open ('dataCard_' + regionType +'.txt', mode = 'wb') as fOut:
            ...
            fOut.write('bin hh_{0}\n'.format(regionType))
            #fOut.write('observation {0}\n'.format((ordDict['data_obs'])[rate]))
            fOut.write("#above is obtained using #hhMt_long->Integral() of root -l muon2016data.root from:\n")
 ...
 #               fOut.write(systParamLineDY)
  #              fOut.write('\n')
   #         elif 'SR' in regionType:
            ...
            fOut.write('\n')
            #else:
             #   print 'smth is wrong writing parapLine to datacard, please check.'
              #  sys.exit(1)
            fOut.write('\n')
            fOut.write(lumiLine)
            fOut.write('\n')
...
# bTag      lnN               1.10       1.10      -       -       -       -       -
# trigger   lnN               1.10       1.10      -       -       -       -       -
# bgFloat   lnU                 -          -     2.00     2.00    2.00    2.00    2.00
...
#                if 'h' in plotName:
 #                   plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio = dataOrNot, plotData = dataOrNot)
  #              else:
                print 'will do "Stack"'
                if 'hh' in plotName:
                    plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio=dataOrNot, plotData = dataOrNot, massPoint=massPoint, bdtCut=bdtCut, regionType=regionType)
                elif 'bdt' in plotName:
                    plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio=True, plotData = dataOrNot, massPoint=massPoint, bdtCut=bdtCut, regionType=regionType)
                else:
                    pass
                ...
                plots[p].overlayHists(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, massPoint=massPoint)
                    ...
                    plots[p].makeDataCards(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, regionType=regionType, massPoint=massPoint, applyBR=applyBR)
            else:
