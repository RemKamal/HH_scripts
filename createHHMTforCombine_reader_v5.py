...
for sample, name in dictOfFiles.items():
    ...
    centinel = maxValue-1
    histos = {
        'hhMt'      :ROOT.TH1F('hhMt', ';"HiggsHiggson" transverse mass [GeV]; Events', (maxValue - minValue)/20, minValue, maxValue),
        ...
        'hhMt_long_syst'      :ROOT.TH1F('hhMt_long_syst', ';"HiggsHiggson" transverse mass [GeV]; Events', (maxValue - minValue)/20, minValue, maxValue),
...
#            for v in myVars:
 #               print  v + '[0] = entry.' + v
        ...
        leps_SF = entry.weight_SF_TRK[0] * entry.weight_SF_LooseIDnISO[0] * entry.weight_SF_Dilep[0]
        btags_SF = entry.Jet_btagWeightCMVAV2[entry.hJCMVAV2idx[0]] * entry.Jet_btagWeightCMVAV2[entry.hJCMVAV2idx[1]] 
        weight_leps_btags = leps_SF * btags_SF
        extraWeight = weight_leps_btags 
        ...
        histos['hhMt'].Fill(min(entry.hhmt, centinel), eqLumi)
        histos['hhMt_long_syst'].Fill(min(entry.hhmt, centinel), eqLumi*9.05)
