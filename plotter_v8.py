...
#////////////////////////
    ...
    def doStack(self, outDir,lumi,noScale=False,saveTeX=False, ratio=False):
...
#        frame.Draw('')  #check here
        ...
        if self.data is not None : 
            if 'h' not in self.dataH.GetName():
    ...
    for p in plots : 
        ...
        if not opt.silent : 
            if opt.kind == 'Stack':
                ...
                plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio=True)
            elif opt.kind == 'StackNoRatio':
                #plots[p].simpleStackNoRatio(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX)
                plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX)
