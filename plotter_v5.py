    ...
    def add(self, h, title, color, isData):
        ...
        if isData:
            try:
                ...
                self.dataH.SetMarkerStyle(1) #20
...
#check
        ...
        # legend
        leg = ROOT.TLegend(0.45, 0.85-0.02*max(len(self.mc)-2,0), 0.98, 0.94)        
...
def main():
    ...
    #            plots[key].add(h=obj,title=sample[3],color=sample[4],isData=sample[1])
                plots[key].add(h=obj,title=sample[3],color=sample[4],isData=sample[6])
        except:
