...
whenMade = '17_3_' #  _ at the end is important!                                   
outputDirDate = '_dec10/'
...
if not os.path.exists(dest):
    os.makedirs(dest)
pref, postf = 'analysis_nov', '_total_SR_minitrees_inpb_wBR'
...
#    if mass  > 450 and bdtCut < 0.99: exit(1)
    massRegion = 'low' #'low' if bdtCut < 0.99 else 'high' if bdtCut >= 0.99 else None
...
def parallAddShapes(cmd):
    ...
    #mm_CRDY_CMS_eff_b_lightUp.input.root
    if systUnc != 'nominal':
...
if doEles and doMuons:
    ...
    #only one element in the list, take it with ':1'
    nomin_ee_low = [dirs[:1], ['nominal'], physRegion, ee_bdtCuts[:1], masses[:1]]
    nomin_ee_high = [dirs[:1], ['nominal'], physRegion, ee_bdtCuts[:1], masses[:1]]

    nomin_mm_low = [dirs[:1], ['nominal'], physRegion, mm_bdtCuts[:1], masses[:1]]
    nomin_mm_high = [dirs[:1], ['nominal'], physRegion, mm_bdtCuts[:1], masses[:1]]

    ee_low = [dirs[:1], firstPass_ee_systUnc, physRegion, ee_bdtCuts[:1], masses[:1]]
    ee_high = [dirs[:1], firstPass_ee_systUnc, physRegion, ee_bdtCuts[:1], masses[:1]]

    mm_low = [dirs[:1], firstPass_mm_systUnc, physRegion, mm_bdtCuts[:1], masses[:1]]
    mm_high = [dirs[:1], firstPass_mm_systUnc, physRegion, mm_bdtCuts[:1], masses[:1]]
