...
visualization_mult_factor = 2000
badRoots = [
	'zmass',
	'dR_bjets'
	'hmass0',
...
def createPostFitPlot(ifile):
	...
	bg_hists = [x.Clone() for x in mc_hists] #deepcopy(mc_hists)
	...
	for h in bg_hists:
		for idx, bin in enumerate(h):
			if h.GetBinContent(idx) < 0.00001:
	...
	for idx, bin in enumerate(copyTotalProcs):
		if copyTotalProcs.GetBinContent(idx) < 0.00001:
...
# print "Total data: ",data.Integral()
	...
	for i in range(0, mc_total_uncUp.GetNbinsX()):
		...
		e = 0.
		#if mc_total.GetBinContent(i + 1) != 0 and mc_total.GetBinContent(i + 1) < 0.00001: continue
		#if data.GetBinContent(i + 1) != 0 and data.GetBinContent(i + 1) < 0.00001: continue
		if mc_total.GetBinContent(i + 1) > 0.00001 and data.GetBinContent(i + 1) > 0.00001:
