...
name = '1pb_noBDTcut_inCRs_Zm10p10_Hm20p20/'
...
for sample, name in dictOfFiles.items():
    ...
    for entry in sample.tree:
        ...
        if physRegion == 'SR':
            if entry.zmass < 81 or entry.zmass > 101: continue
            if entry.hmass1 < 105 or entry.hmass1 > 145: continue
        elif physRegion == 'CRDY':
            if entry.zmass < 81 or entry.zmass > 101: continue
            if entry.hmass1 >= 105 and entry.hmass1 <= 145: continue
        elif physRegion == 'CRTT':
            if entry.zmass <= 101: continue
            if entry.hmass1 < 105 or entry.hmass1 > 145: continue
        else:
