...
date = 'oct15_1'
version = '_inpb_wBR'
jsons = [ #'test_bbZZ.json',
          #'test_bbZZ_noTTnDY.json',
    'toMakeDataCard_samples_1pb_bbZZ_muons_with_bbVV_v2.json',
    'toMakeDataCard_samples_1pb_bbZZ_eles_with_bbVV_v2.json']
...
date += '_muons' if channel ==muons else '_eles' if channel ==electrons else ''
...
for typ in typeList:
    ...
    totalCmd = "nohup python runSimpleAn_v17_" + regionArgument  + "_indev_v5.py  -i samplesSept21  -j data/" + json + " -w analysis_" + date + "_total_" + typ + '_' + regionArgument + version + " -r " + typ + " -n 16 -t " + runTyp + " --channel2run " + str(channel) + " &> log_" + date + "_total_" + typ + '_' + regionArgument + version + ".txt&"
    print
