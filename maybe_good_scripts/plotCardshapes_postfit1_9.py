...
# print "Total data: ",data.Integral()
	...
	for ext in [".png"]:#, ".pdf", ".C", ".root"]:
		canv.SaveAs("{0}_{1}_plot_{2}{3}".format(name, do_prefit_or_postfit, date, ext))
...
def main():
	...
	for fil in files:
		...
		# 		#'hmass0' not in fil and 'leps' not in fil:
                ...
                #if 'post' in fil:
		#createPreFitPlot(fil, do_prefit_or_postfit)        
