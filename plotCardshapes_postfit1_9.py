...
do_SR_BDTSideband = True
plotData_in_SR = False if not do_SR_BDTSideband else True
#plotData_in_SR = True

includeAbsentSignal = False
tolerate = 0  #0.0001 #extra 0 should be added?!
...
def createPreFitPlot(ifile):
	...
	tolerance  = 0.005 if ('hh' in name or 'dR' in name or 'zpt' in name) else tolerate
...
# print "Total data: ",data.Integral()
	...
	for ext in [".png"]:#, ".pdf", ".C", ".root"]:
		canv.SaveAs("{0}_prefit_plot_{1}{2}".format(name, date, ext))
...
def main():
	...
	files = CRDYfiles + CRTTfiles
	if do_SR_BDTSideband:
		files = SRfiles
