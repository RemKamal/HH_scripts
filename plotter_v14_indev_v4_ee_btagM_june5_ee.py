...
class Plot(object):
    ...
    def add(self, h, title, color, isData, isSignal, unc):
        h.SetTitle(title)

        # if not title.startswith('Data'):
        #     #was designed for 'hhMt'
        ...
        print '*'*2000
        if sampleType != 'data_obs' and ('nominal' not in unc  or '' not in unc):
...
#////////////////////////
    ...
    def doStack(self, outDir,lumi,noScale=False,saveTeX=False, ratio=False, plotData = True, massPoint=0, bdtCut = 0., regionType = None, channel2run = None, doPostFit=False):
        ...
        try:
            DY_norm = 1#df[channel2run]['DY'][massPoint]
            TT_norm = 1#df[channel2run]['TT'][massPoint]
...
#//////////////////////////
    def makeDataCards(self, outDir,lumi,noScale=False,saveTeX=False, regionType=None, massPoint=None, applyBR = True, channel2run = None, plotName = None):
...
#        for key,value in ordDict.items():
 #           print(key, ":", value)
  #          print 'type(key) is ', type(value[2]) 
        ...
        #outF = ROOT.TFile.Open('hists_' + bin + '.root','recreate')
        ...
        if True:
            ...
            with io.open ('dataCard_' + regionType + '_' + self.name + '.txt', mode = 'wb') as fOut:
                fOut.write('#  HHtobbZZto2b2l2nu dataCard for ' + regionType + ' \n')
            ...
            #fOut.write('imax 1 number of channels, switch to two when add EGamma data?\njmax {0} number of MC samples minus one\nkmax * number of nuisance parameters (sources of systematical uncertainties)\n'.format(   len(ordDict) -2  ))# -1 for data_obs, -1 for each signal sample
                ...
                fOut.write('#  shapes * * ' + plotName + '_' + bin + '.input.root $CHANNEL/$PROCESS $CHANNEL/$PROCESS_$SYSTEMATIC\n')
                #fOut.write('shapes * * ' + bin + '_Mt.root $PROCESS $PROCESS_$SYSTEMATIC\n')
                fOut.write('shapes * * ' + plotName + '_' + bin + '.input.root {0}/$PROCESS {1}/$PROCESS_$SYSTEMATIC\n'.format(bin, bin))
                fOut.write('#-----------------------------------------------------------------------------------\n')
...
def main():
        ...
        try:
            for tkey in fIn.GetListOfKeys():
                ...
                if 'makeDataCards' in opt.kind:
                    if 'mass' in key or 'hhMt' in key or 'zpt' in key or 'hpt' in key or 'dR_' in key or ('bdt' in key) or 'met_pt' in key:
                        pass
                        #print 'key is ', key
    ...
    #            plots[key].add(h=obj,title=sample[3],color=sample[4],isData=sample[1])
                plots[key].add(h=obj,title=sample[3],color=sample[4],isData=sample[1], isSignal=sample[6], unc = systUnc)
 ...
 #                   plots[p].doStack(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, ratio = dataOrNot, plotData = dataOrNot)
  #              else:
                ...
                plots[p].overlayHists(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, massPoint=massPoint)
                    ...
                    plots[p].makeDataCards(outDir=outDir,lumi=opt.lumi,saveTeX=opt.saveTeX, regionType=regionType, massPoint=massPoint, applyBR=applyBR, channel2run=channel2run, plotName=plotName)
            else:
        ...
        if 'makeDataCards' in opt.kind:
            if systUnc == 'nominal':
                plots[p].appendTo(outDir+'/{0}_{1}.input.root'.format(plotName+'_nominal', channel2run+ '_' + regionType ), makeDataCards=True, channel2run = channel2run, regionType=regionType)
            else:
                plots[p].appendTo(outDir+'/{0}_{1}.input.root'.format(plotName, channel2run+ '_' + regionType ), makeDataCards=True, channel2run = channel2run, regionType=regionType)
        else:
