...
for mm_d, ee_d in zip(mm_dirs, ee_dirs):
    ...
    tmpDir =  'combinedCards_' + str(mass) + ee_extra_if_bdt + mm_extra_if_bdt 
    print 'tmpDir =', tmpDir
    ...
    if os.path.exists (tmpDir):           
        ...
        commandFit = 'combine -M Asymptotic -t -1 -v 3 --run blind -m ' + str(mass) + ' combinedCard_' +  str(mass) + '.txt >& log_combined_M' + str(mass) + '.txt&' 
        commandFit_ML = 'combine -M MaxLikelihoodFit -v 3 -m ' + str(mass) + range_ + ' combinedCard_' +  str(mass) + '.txt >& log_combined_M' + str(mass) + '_ML.txt&' 
