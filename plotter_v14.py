...
#        leg = ROOT.TLegend(0.25, 0.86, 0.88, 0.94)  
        ...
        for hS in self.mcSignal:
            ...
            if 'WW' in self.mcSignal[hS].GetTitle():
                ...
                signalMultFactor = self.dataH.GetMaximum()/self.mcSignal[hS].GetMaximum()
                if makeVisible:
...
#        frame.Draw('')  #check here
        ...
        # THIS is very you control height of the distribution on the canvas (before used maxY*1.3)
        if 'hh' in self.name:
            stack.SetMaximum(maxY*5.5)
        else:
            stack.SetMaximum(maxY*4.5)
...
#//////////////////////////
    def makeDataCards(self, outDir,lumi,noScale=False,saveTeX=False, regionType=None, massPoint=None, applyBR = True):
        ...
        #bbvv:     masses = [260, 270, 300, 350, 400, 450, 600, 650, 900, 1000]
        ...
        for h in ndic:
            ...
            elif '_WW' in ndic[h].GetName():#[nameEnding_beg+1:]:
                dicRates[ndic[h].GetName()] = [-1, round(tmpRate, 2),ndic[h]]
            elif '_zz' in ndic[h].GetName():#[nameEnding_beg+1:]:                                                    
                dicRates[ndic[h].GetName()] = [-2, round(tmpRate, 2),ndic[h]]
...
#        for key,value in ordDict.items():
 #           print(key, ":", value)
  #          print 'type(key) is ', type(value[2]) 
        ...
        ggList = ['signal_zz', 'signal_vv', 'signal_ZZ', 'signal_WW', 'TT']
...
#\n')
        ...
        if printStrings:
            print 'Doing {0} GeV datacard'.format(massPoint)
...
def main():
    ...
    if massPoint is None or massPoint ==0:
        ...
        sys.exit(1)
    extraName = 'shapes_' + 'mumu' if opt.channel2run == 0 else "elel" if opt.channel2run == 1 else "" 
    #read plots 
    plots={}
