...
inpFiles = [
    #notice the prefix - var name, is skipped

    'ee_CRDY_postfit.root',
...
def getCHI2andFixZeroValues(var):
    for fil in inpFiles:
        ...
        print "name=%s, fil=%s, region=%s, typ=%s, innerDirName=%s, syst=%s" % (f_input_shapes_TH1.GetName(), fil, 1, 1, innerDirName, 1)
        ...
        sumProcesses = [x.Clone() for x in hists if 'TotalProcs' in x.GetName()][0]
        print 'sumProcesses', sumProcesses.Print('all')
        data = [x.Clone() for x in hists if 'data' in x.GetName()][0]
        print 'data', data.Print('all')
        print data.Chi2Test(sumProcesses , "P")
        print "Chi2/NDF = ", data.Chi2Test(sumProcesses , "UWCHI2/NDF")
