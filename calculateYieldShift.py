...
def getNormalizedYieldVariation(listOfHists, sample, nuis):
    print 'listOfHists', listOfHists
    print 'sample', sample
    print 'nuis', nuis
    nameUp = sample + '_' + nuis+"Up"
    nameNom = sample 
    nameDown = sample + '_' + nuis+"Down"

    print 'nameUp', nameUp
    print 'nameNom', nameNom
    print 'nameDown', nameDown

    histUp = [x for x in listOfHists if sample + '_' + nuis+"Up" in x.GetName()][0].Integral() 
    histNominal = [x for x in listOfHists if sample in x.GetName() and "Up" not in x.GetName() and "Down" not in x.GetName()][0].Integral()
    histDown = [x for x in listOfHists if sample + '_' + nuis+"Down" in x.GetName()][0].Integral()
    print 'histUp', histUp
    print 'histNominal', histNominal
    print 'histDown', histDown
    value = None
    if histUp > 0 and histNominal >0 and histDown > 0:
        value = abs(histUp - histDown)/(2.0*histNominal)
    elif histNominal ==0:
        value = 0
    elif histUp > 0 and histNominal >0 and histDown == 0:
        value = abs(histUp - histNominal)/(1.0*histNominal)
    elif histUp == 0 and histNominal >0 and histDown == 0:
        value = abs(histDown - histNominal)/(1.0*histNominal)
    else:
        print 'smth is wrong in getNormalizedYieldVariation, exiting....'
        print 'histUp', histUp
        print 'histNominal', histNominal
        print 'histDown', histDown
        sys.exit(1)
    ...
    print value
    return round(value*100., 1) # return rounded value in percents

def collectHists(inFile):
    print 'inFile', inFile
    
    hists = []
    fin = TFile(inFile)
    fin.cd(channelIn + "_SR")
    ...
    uncTypes = [x.GetName() for x in hists if "TT" in x.GetName()]
    print 'uncTypes', uncTypes

    # ee_nuisances =  ['CMS_eff_e_triggerUp', 'CMS_eff_e_IDUp', 'CMS_res_jDown', 'CMS_res_jUp', 'CMS_eff_e_triggerDown', 'CMS_eff_e_trackerUp', 'CMS_btag_heavyDown', 'CMS_scale_jUp', 'CMS_scale_jDown', 'CMS_btag_heavyUp', 'CMS_btag_lightUp', 'CMS_eff_e_IDDown', 'CMS_btag_lightDown', 'CMS_eff_e_trackerDown'] 

    # mm_nuisances =  ['CMS_eff_m_triggerUp', 'CMS_eff_m_IDUp', 'CMS_eff_m_ISOUp', 'CMS_res_jDown', 'CMS_res_jUp', 'CMS_eff_m_triggerDown', 'CMS_eff_m_trackerUp', 'CMS_btag_heavyDown', 'CMS_scale_jUp', 'CMS_scale_jDown', 'CMS_btag_heavyUp', 'CMS_btag_lightUp', 'CMS_eff_m_IDDown', 'CMS_eff_m_ISODown', 'CMS_btag_lightDown', 'CMS_eff_m_trackerDown']

    ee_nuisances =  ['CMS_eff_e_trigger', 'CMS_eff_e_ID', 'CMS_res_j', 'CMS_eff_e_tracker', 'CMS_scale_j', 'CMS_btag_heavy', 'CMS_btag_light'] 

    mm_nuisances =  ['CMS_eff_m_trigger', 'CMS_eff_m_ID', 'CMS_eff_m_ISO', 'CMS_res_j',  'CMS_eff_m_tracker',  'CMS_scale_j', 'CMS_btag_heavy', 'CMS_btag_light']

    nuisances = ee_nuisances if "ee" == channelIn else mm_nuisances
    
    samples = ["signal_hzz", "signal_hww", 
               "TT", "DY", "ZH", "ST", "VV"]
    ...
    for name in samples:
        ...
        print 'name', name
        sample = [x for x in hists if name in x.GetName()]
        storage[name] = sample
    #print 'DY', storage['DY']

        
    nuis_DY = NuisanceCollection() 
    nuis_TT = NuisanceCollection()
    nuis_signal_hzz = NuisanceCollection()
    nuis_signal_hww = NuisanceCollection()
    for idx, nuis in enumerate(nuisances):
        #if idx >1: continue
        dYield_DY = getNormalizedYieldVariation(storage['DY'], 'DY', nuis)
        dYield_TT = getNormalizedYieldVariation(storage['TT'], 'TT', nuis)
        dYield_signal_hzz = getNormalizedYieldVariation(storage['signal_hzz'], 'signal_hzz', nuis)
        dYield_signal_hww = getNormalizedYieldVariation(storage['signal_hww'], 'signal_hww', nuis)
        setattr(nuis_DY, nuis, dYield_DY)
        setattr(nuis_TT, nuis, dYield_TT)
        setattr(nuis_signal_hzz, nuis, dYield_signal_hzz)
        setattr(nuis_signal_hww, nuis, dYield_signal_hww)

    print
    print '{0}_nuis_DY ='.format(channelIn)
    pp(nuis_DY.__dict__)
    print
    print '{0}_nuis_TT ='.format(channelIn)
    pp(nuis_TT.__dict__)
    print
    print '{0}_nuis_signal_hzz ='.format(channelIn)
    pp(nuis_signal_hzz.__dict__)
    print
    print '{0}_nuis_signal_hww ='.format(channelIn)
    pp(nuis_signal_hww.__dict__)
    print
...
def main(argv):
   ...
   outputfile = ''
   try:
      opts, args = getopt.getopt(argv,"hi:o:",["inputFile=","ofile="])
   except getopt.GetoptError:
      print 'test.py -i <inFile> -o <outputfile>'
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
         print 'test.py -i <inFile> -o <outputfile>'
   ...
   global channelIn
   channelIn = "ee" if "ee" in inFile else "mm" if "mm" in inFile else None
