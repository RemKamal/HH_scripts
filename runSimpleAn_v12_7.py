...
"""
def runSimplestAn(inFileURLorList, outFileURL, xsec=None, denominator=None, isSignal = False, splitSignal = None, whichChannel = 0, physRegion = None):
    
    bazinga('before list of binLabels')
    bazinga('analyzing region: ')
    bazinga(physRegion)

    binLabels = ['no cut', '>=2b-jets', '90<Hbb<150',  '2leps', '76<Z<106', 'met>20','hhMt>250']
        ...
        if xsec!=0 and countWeighted != 0: 
            ...
            if nBJets > 0:
                ...
                if nBJets >1 and 'b'not in physRegion:
                    evWgt *= bJetsCSV[1]
                    #apply only a btag CSV SF if it is CR DY with 0 or 1 b-jets
...
# build HH candidate and draw mass transverse  
        ...
        histos['btagCSV0_beforeMETcut'].Fill(bJetsCSV[0])
        histos['btagCSV1_beforeMETcut'].Fill(bJetsCSV[1])
...
#============================
	...
	dEta_bjets = bJetsP4[0].Eta() - bJetsP4[1].Eta()
        ...
        histos['btagCSV0'].Fill(bJetsCSV[0])
        histos['btagCSV1'].Fill(bJetsCSV[1])
# tree->Scan("tree.Jet_btagCSV[tree.hJCidx[0]]")
...
#with open (fileWithTrees, mode = "rt") as f:
            ...
            taskList.append( (fileWithTrees,outFileURL,xsec, denominator, isSignal, splitSignal, whichChannel, physRegion) )
    else:
        inFileURL  = inGlobalFile
        xsec = None
        print '************* this is debug run*************'
        outFileURL = '%s/%s.root' % (opt.outDir,"plots")
        taskList.append( (inFileURL,outFileURL,xsec, denominator, isSignal, splitSignal, whichChannel, physRegion) )
    ...
    if opt.njobs == 0:
        for inFileURL, outFileURL, xsec, denominator, isSignal, splitSignal in taskList:
            runSimplestAn(inFileURL=inFileURL, outFileURL=outFileURL, xsec=xsec, denominator=denominator, isSignal = isSignal, splitSignal=splitSignal, whichChannel=whichChannel, physRegion=physRegion )
    else:
...
#additions on April2
#1. no met cut
#2. if debug run->use 1./20 of signal events and 1./600 of BG events
#3. k-factor 1.23 for all 4 DY samples
